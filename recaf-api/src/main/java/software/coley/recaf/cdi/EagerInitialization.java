...
public @interface EagerInitialization {
	 ...
	 * Having the value set to {@link InitializationStage#CONTAINER_DEPLOY} will result in the bean and <i>all</i> of
	 * its dependencies being created as soon as the application begins. For beans dealing with UI capabilities this
	 * will likely lead to problems. For those situations, use {@link InitializationStage#UI_INITIALIZE} to delay
	 * initialization until after the UI has been populated.
	 * <br>
	 * {@link WorkspaceScoped} beans do not need to ever change this value since they are only created/scoped to the
	 ...
	 */
	InitializationStage value() default InitializationStage.CONTAINER_DEPLOY;
}
